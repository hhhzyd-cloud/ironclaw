...
Built-in host HTTP returns redirect responses without following them. This preserves the #3088 redirect invariant for the current V1 surface by never forwarding credentials to a redirected target. A future redirect-following transport must re-run network policy and credential target policy for every hop before reinjecting credentials.
