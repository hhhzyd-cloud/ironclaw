...
impl KeyStore for SecretsKeyStore {
    ...
    async fn consume(
        ...
        let bytes = hex_decode(decrypted.expose())
            .map_err(|reason| KeyStoreError::Decryption { reason })?;
...
fn hex_decode(s: &str) -> std::result::Result<Vec<u8>, String> {
    ...
    let mut i = 0;
    while i < bytes.len() {
        let hi = hex_nibble(bytes[i]).ok_or_else(|| "invalid hex digit".to_string())?;
        let lo = hex_nibble(bytes[i + 1]).ok_or_else(|| "invalid hex digit".to_string())?;
        out.push((hi << 4) | lo);
        i += 2;
    }
    Ok(out)
...
mod tests {
    ...
    async fn bind_then_consume_round_trips_key_bytes() {
        ...
        store
            ...
            .unwrap();
        let consumed = store
            .consume(&s, &ChainKeyId::new("eip155:1"), ChainFamily::Evm)
            .await
            .unwrap();
    ...
    async fn wrong_family_rejected_before_decryption() {
        ...
        let err = store
            .consume(&s, &ChainKeyId::new("eip155:1"), ChainFamily::Solana)
            .await
            .unwrap_err();
        assert!(matches!(
            err,
