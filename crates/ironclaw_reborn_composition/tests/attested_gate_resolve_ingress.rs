...
use ironclaw_chain_signing::{ChainKeyId, SecretsKeyStore, evm};
...
use ironclaw_reborn_composition::{LocalDevAttestedComposition, RebornAttestedContinuation};
...
/// Deterministic 32-byte hash the wallet attests to.
fn bound_hash() -> ApprovedTxHash {
    ApprovedTxHash::from_bytes([0x5au8; 32])
}

/// Unused-on-the-Solana-path decoded tx required to populate the binding.
fn placeholder_decoded() -> DecodedTransaction {
    evm::decode_eip1559(&TxEip1559 {
        chain_id: 11155111,
        nonce: 1,
        gas_limit: 21_000,
...
fn binding(account_hex: &str, hash: ApprovedTxHash) -> AttestedGateBinding {
    AttestedGateBinding {
        ...
        chain: ChainKeyId::new("solana:mainnet"),
        scope: ResourceScope {
            tenant_id: TenantId::new(TENANT).unwrap(),
