...
mod postgres {
    ...
    impl SigningLedger for PostgresSigningLedger {
        ...
        async fn advance(
            ...
            // with a conditional UPDATE keyed on that same `from`. The CAS makes
            // the validate+write atomic against concurrent advances.
            let from = self.state(gate_ref).await?;
            ...
            // Lost the CAS: the row changed under us (or vanished). Re-read and
            // report against the now-current state.
            let current = self.state(gate_ref).await?;
            Err(LedgerError::InvalidTransition { from: current, to })
        }
    }
}
...
mod libsql_backend {
    ...
    impl SigningLedger for LibSqlSigningLedger {
        ...
        async fn advance(
            ...
            let current = self.read_state(&conn, gate_ref).await?;
            Err(LedgerError::InvalidTransition { from: current, to })
        }
    }
}
