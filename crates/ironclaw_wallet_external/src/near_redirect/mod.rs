...
pub fn encode_near_redirect_proof(payload: &NearRedirectProofPayload) -> Vec<u8> {
    ...
    // by any deterministic round-trippable encoding.
...
impl SigningProvider for NearRedirectSigningProvider {
    ...
    async fn verify_resume(
        ...
        // 3. Account binding (threat #4): the claimed NEAR account must equal the
        //    bound account before we trust any signature it produced.
        let bound_account = context.key_or_account_id.as_str();
        if payload.account_id != bound_account {
...
mod tests {
    ...
    fn proof_payload_round_trips_through_opaque_bytes() {
        ...
        let bytes = encode_near_redirect_proof(&payload);
